...
#let zds-discussion = "The zenfmt server: the zencli and zenserve libraries, REST and streaming conversion API, two-role auth on embedded zaxonlite, observability, and a Zig WebAssembly interface styled with daisyUI"
#let zds-labels = ("server", "api", "security", "web",)
...
`PUT /api/v1/convert` with Markdown and structured reports, and — when the
deployment warrants it — authenticates users, records an audit trail, and
serves a small administration interface.
...
library — `std.http.Server`, `std.net`, `std.crypto`, `std.Io` — organized
...
keys, and the audit log live in zaxonlite — the embedded replicated SQLite
library from the paxos-zig monorepo, consumed here as a pinned package in
...
retrieval stores all need the same operation — bytes of some office format
in, Markdown and a manifest out — and none of them want to link a native
...
- every failure is an Elm-style report — what happened, where, what the
  server did instead, what you can do — rendered as structured JSON for
...
- *open mode*: the stateless posture — no accounts, no data directory, no
  persistence; comparable to a default `tika-server`;
- *secure mode*: the administered posture — accounts, sessions, API keys,
...
- *role*: one of exactly two authorization levels, `administrator` and
  `user`;
- *principal*: the authenticated identity of a request — a session, an API
  key, or the implicit anonymous user of open mode;
...
- *glue*: the fixed JavaScript adapter between browser APIs and the ui
  module — the only JavaScript in the product;
...
#table(
  ...
  [build graph], [`build/server.zig`, the new root steps, `fmt_paths` and
    ...
    `tls = false`, and the `-Dserver` build option.],
  ...
  [benchmark], [The loopback throughput benchmark following the
    `benchmarks/` pattern, with Apache Tika server as the competitor
    runner.],
)
...
the server as one monolithic application would leave the next service —
and insan.ai has more services planned — to start from zero again. The
...
- `Flag`: the existing declaration shape — long name, short name, value
  arity, help line — moved verbatim;
...
answer HTTP, observe itself, and authenticate principals — and nothing
...
- One acceptor thread owns the `std.net` listener.
...
- Connection state is a static array of `max_connections` slots (default
  128) allocated once at startup — no per-connection heap allocation for
  the slot itself, TigerBeetle-style. A connection beyond capacity
...
Matching is a bounded segment walk — no regular expressions, no dynamic
registration, no allocation. A route table with duplicate method/path
...
line takes typed fields, not preformatted strings, which is what makes
...
(liveness). `/readyz` consults registered readiness checks — the worker
pool and, in secure mode, a store ping — and answers `503` with the
...
- `Role`: `anonymous`, `user`, `administrator` — ordered, so a route
...
- session and API-key descriptors as plain structs plus a `Store`
  interface (vtable) the application implements over zaxonlite — lookup
  by digest, insert, touch, revoke — so zenserve stays storage-free;
...
#table(
  ...
  [`GET/POST /api/v1/users`], [administrator], [secure], [List accounts;
...
- The input name — used for detection exactly as `InputSpec.bytes`
  requires — comes from, in priority order: the `filename` of a multipart
...
A failed conversion returns the failure envelope — `status: "failed"`,
`reports`, `exit_class`, no artifact — with the HTTP status mapped from
...
#table(
  ...
  [`server.open-network-bind`], [—], [Startup warning, not an HTTP
...
repository — `zig fetch --save
https://github.com/insanai/zaxonlite/archive/refs/tags/v0.3.0.tar.gz` —
built with `.tls = false`, which compiles out the OpenSSL mTLS transport
...
+ `durability.setSyncMode(.full)` — explicit, so Linux gets the same
  fsync barrier Darwin defaults to; set once before any store I/O.
+ `Node.open(gpa, io, .{ .directory = data_dir })` — empty `members`
  yields the single-member configuration: commit and apply complete
...
not audited per request — the audit table records administrative fact,
...
#table(
  ...
  [`zenfmt_http_connections_active`], [gauge], [—],
  ...
  [`zenfmt_conversion_duration_seconds`], [histogram], [—],
  [`zenfmt_conversion_input_bytes`], [histogram], [—],
  [`zenfmt_conversion_artifact_bytes`], [histogram], [—],
  [`zenfmt_conversions_active`], [gauge], [—],
  [`zenfmt_auth_failures_total`], [counter], [`kind` (password, token,
    key)],
  [`zenfmt_sessions_active`], [gauge], [—],
  [`zenfmt_store_operation_duration_seconds`], [histogram], [`op` (read,
    write)],
  [`zenfmt_store_errors_total`], [counter], [`op`],
  [`zenfmt_events_dropped_total`], [counter], [—],
)

`source_format` and `output_format` label values are the compiled
bundle's format identifiers plus `unknown` — a closed set at compile
...
module — compiled from Zig — that owns every page, every fragment of
...
when the command protocol changes. Every product decision — what a page
shows, how a report renders, what a button does — is Zig, unit-testable
...
instantiation — a mismatch is a load failure, not a guess (the
...
#table(
  ...
  [module → glue (commands)], [`patch` (element id, HTML), `title`,
    ...
    `dialog_open` / `dialog_close`, `clipboard` (text).],
  [glue → module (events)], [`init` (route, mode), `route_change`,
...
badge, modal, stat, toast) on the default light and dark themes via
`prefers-color-scheme`, one accent color, no custom CSS beyond a small
layout file — modern and calm, not a dashboard product.
...
line for the conversion API — the API, not the interface, is the
...
#table(
  ...
  [`/`], [user], [The converter: drop zone and file picker, output format
    ...
    buttons for artifact and resources.],
  ...
  [`/admin/users`], [administrator], [Account table with role badges;
    create, role change, disable, password reset, delete — each a modal
...
```text
┌──────────────────────────────────────────────────────────────┐
│ navbar: zenfmt · Convert · [Account] [Admin] [Log out]       │
├──────────────────────────────────────────────────────────────┤
│ ┌───────────────────────────────┐  ┌───────────────────────┐ │
...
resource bytes. Report panels render the Elm-style sections verbatim —
the interface never paraphrases a report.
...
- State-changing interface requests carry a per-session CSRF token in
  the `X-Zenfmt-Csrf` header, attached by the glue to every `fetch`
  command; the server requires it for any cookie-authenticated non-GET
  request. Bearer and API-key requests are CSRF-exempt by construction.
- CSP on every page: `default-src 'self'; script-src 'self'
  ...
  permits wasm compilation only — not JavaScript `eval` — and the only
...
zenfmt serve [options]
  ...
  --behind-proxy         trust X-Forwarded-For from the immediate peer,
...
bounded factors — conversions × (max-body + engine budget) — and the
...
zenserve, or the server — exactly as it never sees `zenfmt_cli` today.
...
#table(
  ...
  [`zig build server-browser-test`], [The interface smoke test through
    ...
    is today).],
  [`zig build benchmark-server`], [Loopback throughput benchmark; with
    Tika installed, the comparison run.],
)

Aggregates: `zig build test` depends on `server-test`; `fmt` and
`fmt-check` cover `server` and `cli/zencli` via `fmt_paths`; the release
gate adds `server-browser-test` and a recorded `benchmark-server` run.
Missing Tika degrades the benchmark to zenfmt-only with an explicit log
line, never a silent skip (the uv-missing precedent from ZDS 0014).
...
reach the port, and executes the most attacker-influenced code in the
monorepo — the format readers — against them. The engine was built for
...
- Asset and interface routes serve only embedded bytes by comptime name —
  no filesystem reads at request time, so path traversal has no target.
...
explicitly out of scope. The supported deployments are: loopback-only
(the default bind), a private network with `--secure`, or a
TLS-terminating reverse proxy with `--behind-proxy`. With that flag the
server trusts `X-Forwarded-For` exactly one hop and marks cookies
...
bombs. The explicit worst case — conversions × (max-body + engine
budget) — is documented so operators size containers deliberately rather
...
recoverable form — Argon2id strings and SHA-256 digests only — and to
set `0o700`. Full-disk or filesystem encryption is the deployment's job
...
- zenserve: router matching and compile-time collision rejection,
  ...
  escaping and truncation, SSE ring semantics — all networkless.
...
- the full role matrix: every route × (anonymous, user, administrator,
  disabled user, must-change-password user) in both modes — small enough
...
login and one admin flow in secure mode, verify keyboard-only
operation and the accessibility checks the site tests already run, and
confirm the `<noscript>` page with JavaScript disabled. Release
requires:
...
converter page, report panels, and downloads; the native golden suite
and the browser smoke test. Exit: open mode is demo-complete offline,
and the ui module's size budget holds.
...
matrix and lifecycle suites. Exit: secure mode passes every gate in this
record's testing section.

== Phase 4: polish and release

NDJSON batch, `--behind-proxy` semantics with the proxy fragments, the
benchmark and its Tika comparison, the book's server chapter, the
...
std has a TLS client only; a TLS server would mean linking OpenSSL — a
...
lookup — at which point the store lookup may as well be the token. The
...
SQLite packaging — pinned amalgamation, WAL discipline, `SharedNode`
concurrency, backup and integrity tooling — and it carries the only
...
design — it degrades to plain forms without JavaScript and needs no
CSP relaxation — but it splits the interface across two authorities:
...
— but in Zig, under this repository's rules, with its state machines
tested natively. Rejected, consistent with ZDS 0015.

== Reusing the ZDS 0015 playground wasm as the interface

The browser playground already converts documents in wasm — client
side, with the 1.7 MiB engine module, outside the server's limits,
...
edits on refresh — a development-loop feature paid for with a compiler
...
A history table would make the server a document store — retention,
privacy, and size questions the design avoids on principle. Operators
...
and cleanup — a second product. The synchronous API with generous
...
+ Is `8998` the right default port, or should the server sit directly on
  Tika's `9998` to be a drop-in replacement where only the URL path
  differs?
...
- Apache Tika Server — the operational baseline this record measures
  against
